...
void setup() {
    ...
    // 1. 🔍 FASE 1: MODO LISTENER PURO (SEM DISCOVERY INICIAL)
    Serial.println("\n📡 FASE 1: Modo Listener Promíscuo...");
    Serial.println("============================================");
    Serial.println("⚡ NOVA ARQUITETURA: Slave em modo listener puro");
    Serial.println("   ├─ Sem discovery inicial (mais rápido)");
    Serial.println("   ├─ Aguardando heartbeat do Master");
    Serial.println("   ├─ Suporta múltiplos canais automaticamente");
    Serial.println("   └─ Discovery apenas se Master não responder");
    Serial.println();
    
    // Inicializar MultiChannelDiscovery mas NÃO executar discovery
    ...
    if (multiChannelDiscovery->begin()) {
        ...
        Serial.println("✅ MultiChannelDiscovery inicializado (standby)");
        Serial.println("💡 Discovery será executado apenas se necessário");
        
        // Tentar carregar canal do cache NVS
        ...
        if (cache.lastChannel > 0 && cache.lastChannel <= 13) {
            ...
            // 🎯 CORREÇÃO: Calcular tempo decorrido com proteção contra overflow de millis()
            if (cache.lastSuccess > 0) {
                unsigned long currentTime = millis();
                unsigned long elapsed;
                
                // Proteção contra overflow de millis() (~49 dias)
                if (currentTime >= cache.lastSuccess) {
                    elapsed = currentTime - cache.lastSuccess;
                } else {
                    // Overflow ocorreu - calcular corretamente
                    elapsed = (ULONG_MAX - cache.lastSuccess) + currentTime;
                }
                
                unsigned long elapsedSeconds = elapsed / 1000;
                
                // Se passou mais de 7 dias, considerar cache inválido
                if (elapsedSeconds > 604800) { // 7 dias em segundos
                    Serial.println("⏱️  Último sucesso: Cache muito antigo (mais de 7 dias)");
                    Serial.println("💡 Cache será resetado na próxima descoberta bem-sucedida");
                } else if (elapsedSeconds > 86400) { // Mais de 1 dia
                    unsigned long days = elapsedSeconds / 86400;
                    unsigned long hours = (elapsedSeconds % 86400) / 3600;
                    Serial.println("⏱️  Último sucesso: " + String(days) + "d " + String(hours) + "h atrás");
                } else if (elapsedSeconds > 3600) { // Mais de 1 hora
                    unsigned long hours = elapsedSeconds / 3600;
                    unsigned long minutes = (elapsedSeconds % 3600) / 60;
                    Serial.println("⏱️  Último sucesso: " + String(hours) + "h " + String(minutes) + "m atrás");
                } else {
                    Serial.println("⏱️  Último sucesso: " + String(elapsedSeconds) + "s atrás");
                }
            } else {
                Serial.println("⏱️  Último sucesso: Nunca (primeira execução)");
            }
        } else {
            // Canal padrão mais comum (canal 6 é mais usado que canal 1)
            masterChannel = 6;
            Serial.println("📶 Usando canal padrão: " + String(masterChannel) + " (mais comum)");
        }
        
    } else {
        Serial.println("❌ Erro ao inicializar MultiChannelDiscovery");
        masterChannel = 6;  // Canal padrão
    }
    
    // Flags de estado
    masterConnected = false;  // Ainda não conectado
    channelSyncCompleted = false;  // Aguardando heartbeat
    
    Serial.println("\n🎯 Slave pronto em modo listener");
    Serial.println("   Aguardando heartbeat do Master no canal " + String(masterChannel) + "...");
    Serial.println("============================================\n");
    
    // 2. Inicializar RelayCommandBox (modo simulação se hardware não disponível)
    Serial.println("🔌 FASE 2: Inicializando RelayCommandBox...");
    if (!relayBox.begin()) {
    ...
    // 3. Inicializar ESPNowBridge no canal detectado
    Serial.println("📡 FASE 3: Inicializando ESP-NOW Bridge...");
    ...
    // 🚨 CRÍTICO: Vincular ESPNowBridge ao MultiChannelDiscovery
    // Isso garante que o MultiChannelDiscovery receba TODAS as mensagens
    if (multiChannelDiscovery) {
        Serial.println("\n🔗 Vinculando ESPNowBridge → MultiChannelDiscovery...");
        espNowBridge->setMessageReceivedCallback([](const uint8_t* mac, const uint8_t* data, int len) {
            // Passar TODAS as mensagens para o MultiChannelDiscovery processar
            if (multiChannelDiscovery) {
                multiChannelDiscovery->handleReceivedMessage(mac, data, len);
            }
        });
        Serial.println("✅ Vínculo estabelecido!");
        Serial.println("   → Todas as mensagens serão processadas por ambos os sistemas");
    }
    
    // ✅ CORREÇÃO CRÍTICA: Forçar canal ESP-NOW após inicialização
    Serial.println("\n🔧 Forçando canal ESP-NOW: " + String(masterChannel));
    esp_wifi_set_channel(masterChannel, WIFI_SECOND_CHAN_NONE);
    delay(100); // Aguardar estabilização
    
    // Verificar se canal foi aplicado
    uint8_t verifyChannel;
    wifi_second_chan_t verifySecondChan;
    esp_wifi_get_channel(&verifyChannel, &verifySecondChan);
    if (verifyChannel == masterChannel) {
        Serial.println("✅ Canal ESP-NOW confirmado: " + String(verifyChannel));
    } else {
        Serial.println("⚠️ Canal ESP-NOW diferente! Esperado: " + String(masterChannel) + 
                     ", Atual: " + String(verifyChannel));
    }
    Serial.println();
    
    // 4. Inicializar SafetyWatchdog
    Serial.println("🛡️ FASE 4: Inicializando SafetyWatchdog...");
    watchdog.begin();
...
void handleSerialCommands() {
    while (Serial.available()) {
        ...
        // ===== 🔍 COMANDOS MULTI-CHANNEL DISCOVERY =====
            ...
            // Discovery multi-canal completo
            if (multiChannelDiscovery) {
                ...
                if (result == DiscoveryResult::SUCCESS) {
                    ...
                    // Reconfigurar ESP-NOW se canal mudou
                    if (channel != WiFi.channel()) {
                        Serial.println("🔄 Reconfigurando para canal " + String(channel) + "...");
                        ...
                        espNowBridge = new ESPNowBridge(&relayBox, channel);
                        if (espNowBridge->begin()) {
                            Serial.println("✅ ESP-NOW reconfigurado!");
                        } else {
                            Serial.println("❌ Erro ao reconfigurar ESP-NOW");
                        }
                    }
                } else {
...
void onMasterFound(uint8_t channel, const uint8_t* masterMac) {
    ...
    // 🚨 CRÍTICO: Se canais diferentes, FORÇAR mudança AGORA!
    if (canalAtual != channel) {
        Serial.println("\n🔄 === SINCRONIZANDO CANAL ===");
        Serial.println("⚠️ Canais diferentes detectados!");
        Serial.println("🔧 Forçando mudança de canal " + String(canalAtual) + " → " + String(channel));
        
        // FORÇAR mudança de canal
        esp_err_t err = esp_wifi_set_channel(channel, WIFI_SECOND_CHAN_NONE);
        if (err == ESP_OK) {
            Serial.println("✅ Canal alterado com sucesso!");
        } else {
            Serial.println("❌ Erro ao alterar canal: " + String(err));
        }
        ...
        if (canalAtual == channel) {
            ...
            Serial.println("⚠️ AVISO: Canal não mudou conforme esperado!");
        }
    } else {
    ...
    // Salvar canal no contexto global (para uso posterior)
...
void performRediscoveryIfNeeded() {
    ...
    static unsigned long rediscoveryInterval = 60000; // 60 segundos
    
    // Verificar se precisa de re-discovery
    bool needsRediscovery = false;
    
    // 🚨 Condição 0: NUNCA encontrou Master (MODO LISTENER PURO)
    // Intervalo curto para encontrar Master rapidamente
    if (!masterConnected && !channelSyncCompleted) {
        needsRediscovery = true;
        // Intervalo progressivo: 10s → 20s → 30s → 45s → 60s
        unsigned long interval = 10000UL + (lastRediscoveryAttempt / 1000UL);
        rediscoveryInterval = min(interval, 60000UL);
        
        // Log informativo (apenas primeira vez)
        static bool firstAttempt = true;
        if (firstAttempt) {
            Serial.println("💡 Modo Listener: Aguardando heartbeat do Master...");
            Serial.println("   Re-discovery automático ativará se não houver resposta");
            firstAttempt = false;
        }
    }
    
    // Condição 1: Master offline (watchdog detectou)
    if (watchdog.isSafetyMode()) {
        needsRediscovery = true;
        rediscoveryInterval = 30000; // 30s em modo de emergência
    }
    
    // Condição 2: Muitos pings falhados
    if (failedPingCount >= maxFailedPings) {
        needsRediscovery = true;
        rediscoveryInterval = 45000; // 45s após falhas
    }
    
    // Condição 3: Master explicitamente desconectado
    if (!masterConnected && channelSyncCompleted) {
        needsRediscovery = true;
        rediscoveryInterval = 20000; // 20s quando desconectado
    }
    
    // Executar re-discovery se necessário
    if (needsRediscovery && (millis() - lastRediscoveryAttempt > rediscoveryInterval)) {
        ...
        // Executar re-discovery (quick scan primeiro)
        DiscoveryResult result = multiChannelDiscovery->rediscoverMaster(true);
        ...
        if (result == DiscoveryResult::SUCCESS) {
            ...
            // Atualizar canal se mudou
            if (newChannel != WiFi.channel()) {
                Serial.println("🔄 Canal mudou! Reconfigurando ESP-NOW...");
                
                // Reinicializar ESP-NOW no novo canal
                if (espNowBridge) {
                    espNowBridge->end();
                    delay(500);
                    delete espNowBridge;
                ...
                espNowBridge = new ESPNowBridge(&relayBox, newChannel);
                if (espNowBridge->begin()) {
                    Serial.println("✅ ESP-NOW reconfigurado para canal " + String(newChannel));
                    masterConnected = true;
                    failedPingCount = 0;
                } else {
                    Serial.println("❌ Erro ao reconfigurar ESP-NOW");
                ...
                Serial.println("✅ Canal inalterado (" + String(newChannel) + ")");
                masterConnected = true;
                failedPingCount = 0;
            }
            
            rediscoveryInterval = 60000; // Voltar ao normal
            
        } else {
            Serial.println("❌ Re-discovery falhou: " + MultiChannelDiscovery::resultToString(result));
            
            // Aumentar intervalo gradualmente
            rediscoveryInterval = min(rediscoveryInterval * 1.5, 300000.0); // Max 5 minutos
