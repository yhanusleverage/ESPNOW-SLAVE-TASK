...
bool ESPNowController::sendDiscoveryBroadcast() {
    ...
    if (!ensureBroadcastPeer()) {
        Serial.println("❌ Peer broadcast indisponível antes do discovery");
        return false;
    }
...
bool ESPNowController::ensureBroadcastPeer() {
    ...
    if (esp_now_get_peer(broadcastMac, &peerInfo) == ESP_OK) {
        if (peerInfo.channel == wifiChannel) {
            return true;
        }
        peerInfo.channel = wifiChannel;
        esp_err_t mod = esp_now_mod_peer(&peerInfo);
        if (mod == ESP_OK) return true;
    }

    esp_now_del_peer(broadcastMac);
    esp_err_t add = esp_now_add_peer(&peerInfo);
    if (add == ESP_OK || add == ESP_ERR_ESPNOW_EXIST) {
        return true;
    }
    Serial.println("❌ ensureBroadcastPeer: " + String(add) + " (0x" + String(add, HEX) + ")");
    return false;
}
